...
\end{figure}
	...
	\begin{frame}
		...
		\begin{itemize}
	\item Также находятся параметры $m_{\chi}$ и $\delta$, при которых предположение о равновесии между аннигиляцией и захватом ($A = \frac{1}{2} C$) перестает быть верным.
\end{itemize}
...
\begin{figure}[!h]
	...
	\caption{Область параметров $m,\delta$ при которых наступает равновесие между $A$ и $C$ ($\chi^*$ --- долгоживущая )}
	}
	\only<2>{\includegraphics[width=0.6\textwidth]{images/EqFD.png}
	\caption{Область параметров $m,\delta$ при которых наступает равновесие между $A$ и $C$ ($\chi^*$ --- короткоживущая )}
	}
